...
- **Operations Dashboard (`/operations`)**: Real-time queue monitoring and management interface providing comprehensive visibility into system health. Features:
  - Live queue metrics via WebSocket with instant updates: Shopify webhook queue and shipment sync queue sizes, oldest message timestamps
  ...
  - WebSocket-based real-time updates eliminate HTTP polling for instant metric updates
