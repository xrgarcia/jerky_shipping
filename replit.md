...
- **Core Features**:
    ...
    - **Shipments Page**: Shipment-centric warehouse interface displaying all shipments with proper SQL LEFT JOIN to orders table, handling shipments without order linkage gracefully. Features 3-column grid layout optimized for warehouse operations: **Left Column** (customer info) - customer name (text-2xl, most important), order number (text-lg), contact info (text-sm); **Middle Column** (shipping details) - address, carrier/service, order age, items count, weight, status badges/tags; **Right Column** (actions) - View Details button, status badges. All data loads immediately including tags and items for quick scanning. Displays return/gift badges, special handling indicators (Saturday delivery, contains alcohol), gift messages, and buyer notes. Includes collapsible items table, orphaned shipment detection, and advanced filters. Navigation to shipment details via dedicated "View Details" button. Uses dual-ID routing strategy (shipmentId ?? id) for seamless navigation with ShipStation IDs or database UUIDs.
