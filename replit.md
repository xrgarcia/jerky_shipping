...
- **Sales Forecasting Projection**: A daily job projects last year's actuals into the `sales_forecasting` table, aligning peak seasons using `peak_season_dates`. Includes product enrichment fields like `is_kit`, `is_assembled_product`, `parent_sku`, and `parent_kit`. On every run (even early-exit runs), it also computes and bulk-updates 4 current velocity fields (`curr_daily_sales_quantity`, `curr_daily_sales_revenue`, `curr_kit_daily_sales_quantity`, `curr_kit_daily_sales_revenue`) using a 14-day trailing average from GCP `sales_metrics_lookup` (yesterday minus 14 days), grouped by sku + sales_channel. Rows without recent sales get NULL. Service: `forecast-generation-service.ts` functions `loadCurrentVelocity()` + `bulkUpdateCurrentVelocity()`.
