...
- **Order Backfill System (`/backfill`)**: Allows importing historical Shopify orders for any date range. Uses `backfillJobs` table, async processing via Upstash Redis queue, and Shopify API rate limiting. Provides UI with date pickers, real-time progress, job history, live queue count display (updates every 2 seconds), and "Purge Queue" button for clearing stale webhooks from Redis queue when needed.
- **Reports Page (`/reports`)**: Business analytics dashboard for shipping managers. Features date range filtering (default: last 30 days), interactive line chart showing order totals by day, and summary widgets for key metrics: total orders, fulfilled orders, shipping revenue (customer-paid shipping costs), total revenue, product value, and returns. Includes detailed **Revenue Breakdown** card showing: Product Sales, Shipping, Tax, Discounts (red, negative), Refunds (red, negative), and Total Revenue (bold, separated). Uses segmented TanStack Query cache keys for efficient data refetching. All date handling uses **Central Standard Time (America/Chicago timezone)** for consistent reporting regardless of server location. Refunded orders are excluded from revenue calculations, chart data, and status counts, but refund amounts are displayed separately in both the Returns widget and Revenue Breakdown.
