...
- **Core Features**:
    ...
    - **PO Recommendations Page**: Purchase order recommendation dashboard querying separate GCP PostgreSQL reporting database. Features sortable/filterable table showing inventory recommendations with calculation steps modal, URL-based filtering for shareability, and automatic defaults to latest stock check date. Implements column whitelisting and input validation for security.
    - **Saved Views System**: Customizable column views for PO Recommendations page stored in `saved_views` table. Users can create multiple named views with custom column visibility/order, filters, and sort settings. Views can be made public for sharing via URL (`?view=<id>`). API endpoints validate input with Zod schemas and enforce ownership (users can only modify/delete their own views). ViewManager component provides view selector dropdown, column settings dialog with drag-to-reorder, and save/share functionality.
    ...
    - **Print Queue System**: Automated shipping label printing workflow with background worker, retry logic, and browser auto-print. Background worker polls every 10 seconds, automatically fetches/creates labels from ShipStation, implements exponential backoff retry (MAX_RETRIES=5: 1min, 2min, 4min, 8min, 16min), and broadcasts WebSocket updates. Frontend auto-print triggers browser print dialog when labels are ready. Includes real-time status updates via WebSockets and operations dashboard integration.
...
- **PO Recommendations Cache Warmer**: Background worker that runs every 6 hours to check for new stock_check_date in the reporting database. Cache entries have no TTL (persist indefinitely) and are only invalidated/refreshed when a new stock_check_date is detected. When new data arrives, old cache is invalidated and new cache is warmed. Pre-warms: (1) single full dataset snapshot (700-900 rows), (2) unique suppliers list, (3) calculation steps for top 50 SKUs by recommended quantity. Frontend fetches snapshot once (24h staleTime) and filters/sorts locally for instant performance. Steps load on-demand per SKU.
