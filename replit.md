...
- **PO Recommendations Cache Warmer**: Background worker that runs every 6 hours to pre-populate Redis cache with latest PO recommendations data from the reporting database. Tracks the most recent stock_check_date and warms cache when new data arrives. Pre-warms: (1) full dataset, (2) all supplier-specific datasets, (3) calculation steps for top 50 SKUs by recommended quantity. Stores last warmed date in Redis (30-day TTL) to prevent redundant database queries. Ensures instant page loads for users by warming cache proactively.
