...
- **Core Features**:
    ...
    - **Shipments Page**: Shipment-centric warehouse interface displaying all shipments with proper SQL LEFT JOIN to orders table, handling shipments without order linkage gracefully. Shows order number from dedicated column when order isn't linked to Shopify. Features warehouse-optimized visual hierarchy: customer name (text-3xl), order number & age (text-xl), full address (text-base), contact/tracking details (text-sm). Tags load immediately and display on main card (not hidden in accordion). Displays status description, customer contact info (email/phone), full shipping address, order age (relative time from order_date), return/gift badges, special handling indicators (Saturday delivery, contains alcohol), total weight, gift messages, and buyer notes. Includes collapsible items table, orphaned shipment detection, and advanced filters. Navigation to shipment details via dedicated "View Details" button (not entire card) to preserve inline controls. Uses dual-ID routing strategy (shipmentId ?? id) for seamless navigation with ShipStation IDs or database UUIDs.
