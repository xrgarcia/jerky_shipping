...
- **Core Features**:
    ...
    - **Shipment Sync Worker**: Dual-path asynchronous processor for enriching shipment data from ShipStation, handling tracking numbers and order numbers. Includes robust rate limit handling and detailed failure logging to a dead letter queue. Automatically triggers Shopify order sync when shipment webhooks arrive for missing orders.
    - **Shopify Order Sync Worker**: Automatic order import system that fetches missing orders from Shopify when shipment webhooks arrive before order webhooks. Features retry logic (max 3 attempts), intelligent queue management, and prevents duplicate imports by checking existing orders. Runs every 8 seconds processing up to 10 orders per batch.
    - **Shipments Page**: Displays all shipments with proper SQL LEFT JOIN to orders table for reliable order/customer data retrieval. UI always displays shipment_id as fallback identifier when order linkage is missing. Replaced previous in-memory join (limited to 50 orders) with efficient SQL query. Features orphaned shipment detection and labeling for shipments missing tracking numbers, ship dates, and shipment IDs. SQL sorting uses NULLS LAST to place orphaned shipments at the end when sorted by ship date. Advanced filters include an "orphaned" checkbox that filters for shipments missing ALL three fields (tracking number, ship date, and shipment ID) using SQL WHERE condition with AND logic.
    ...
    - **Operations Dashboard**: Real-time queue monitoring (Shopify webhook queue, Shipment sync queue, Shopify order sync queue), worker status, backfill job status, Shopify credentials validation, safe webhook re-registration, and data health metrics via WebSockets. Provides interactive failures table and purge actions. Displays three queue cards in responsive grid layout with health status badges.
