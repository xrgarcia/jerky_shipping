...
- **Core Features**:
    ...
    - **Normalized Shipment Data (Production-Ready)**: Shipment items and tags are normalized into dedicated `shipment_items` and `shipment_tags` tables for efficient querying and partial fulfillment tracking. Features:
      - **Batch Migration**: Successfully migrated 1,298 shipments creating 1,965 items (1,500 linked to order_items via external_order_item_id) and 1,692 tags using 500-row batch inserts
      - **N+1 Query Prevention**: `populateShipmentItemsAndTags()` helper uses batch-fetching with `inArray()` to fetch all order items in single query, eliminating N+1 performance issues
      - **Type Safety**: All ID comparisons use String() coercion on both sides to handle mixed numeric/string ShipStation payloads. Database normalized 3,700+ existing rows with CAST to TEXT
      - **Partial Fulfillment**: Shipment items link to order items via `external_order_item_id` (Shopify line item ID), enabling accurate tracking of which items shipped in which shipments
      - **API Endpoints**: Three new routes serve shipment data - `/api/shipments/:id/items`, `/api/shipments/:id/tags`, and `/api/line-items/:id/shipment-items` (with shipment joins)
      - **UI Integration**: ShipmentCard component displays expandable item lists on Shipments page, OrderItemShipmentInfo component shows shipped quantities with badges on Order Detail page
      - **Automatic Population**: All shipment create/update operations in both tracking number and order number paths automatically populate normalized tables
    - **Shipments Page**: Displays all shipments with proper SQL LEFT JOIN to orders table for reliable order/customer data retrieval. Handles shipments without order linkage gracefully, displaying shipment_id as fallback identifier. Features orphaned shipment detection and labeling for shipments missing tracking numbers, ship dates, and shipment IDs. SQL sorting uses NULLS LAST to place orphaned shipments at the end when sorted by ship date. Advanced filters include an "orphaned" checkbox that filters for shipments missing ALL three fields using SQL WHERE condition with AND logic. Shipment cards display expandable item lists from normalized shipment_items table.
