...
- **Purchase Order Snapshots**: `purchase_order_snapshots` table (composite key: stock_check_date + sku) merges `skuvault_products` with `inventory_forecasts_daily` (IFD) from the GCP reporting DB. Snapshots are created on-demand when IFD and internal_inventory dates match and are newer than the latest local snapshot. Service: `server/services/purchase-order-snapshot-service.ts`. API: `/api/purchase-orders/{readiness,create-snapshot,dates,snapshot,config}`. UI: Purchase Orders tab on `/forecasting/purchase-orders` with summary cards, search/filter, CSV export, snapshot date selector, algorithm dropdown, and projection window picker.
  - **IFD field definitions** (`inventory_forecasts_daily` columns stored in the snapshot):
    - `quantity_available` → `available_quantity`: total units available to sell — the primary inventory figure warehouse managers use to know how much stock they have to fulfill orders.
    - `quantity_incoming` → `quantity_incoming`: total units inbound from open supplier purchase orders.
    - `ext_amzn_inv` → `ext_amzn_inv`: total units currently in the Amazon fulfillment warehouse.
    - `ext_wlmt_inv` → `ext_wlmt_inv`: total units currently in the Walmart fulfillment warehouse.
    - `quantity_in_kits` → `quantity_in_kits`: total units that have been bundled into kits or assembled products and are ready to fulfill as a kit.
    - `total_stock` → `total_stock`: sum of all the above fields (`quantity_available + quantity_incoming + ext_amzn_inv + ext_wlmt_inv + quantity_in_kits`). Used as the denominator in the `rec_purchase` formula: `proj_total − total_stock`.
...
- **Growth Factor Adjustment**: A dropdown in the PO toolbar (to the right of the projection algorithm selector) applies a per-SKU growth multiplier to projection columns client-side. Options: `none` (no adjustment), `trend` (trend_factor), `yoy` (yoy_growth_factor), `smart` (max of both). Growth factors are fetched from `GET /api/purchase-orders/growth-factors`, which queries GCP `sales_metrics_lookup` at `MAX(order_date)`, computing a weighted average (weighted by `daily_sales_quantity`) of `trend_factor` and `yoy_growth_factor` per SKU across all channels. Result is cached in Redis (`po:growth-factors`, 1-hour TTL). Multiplication is applied client-side: `adjDirect = round(proj_direct * factor)`, `adjKits = round(proj_kits * factor)`, `adjTotal = adjDirect + adjKits`, `rec_purchase = adjTotal − total_stock`. When a factor is active, the raw (unadjusted) value is shown in muted text below the adjusted value in each projection cell. The selected method is persisted to `purchase_order_config.growth_factor_method`.
