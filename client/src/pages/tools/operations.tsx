...
function RateAnalysisJobsCard() {
  const { toast } = useToast();
  
  const { data: rateAnalysisData, isLoading, refetch } = useQuery<{ jobs: RateAnalysisJob[]; activeJob: RateAnalysisJob | null }>({
    queryKey: ["/api/rate-analysis-jobs"],
    refetchInterval: 5000,
  });
  
  const createJobMutation = useMutation({
    mutationFn: async (preset: string) => {
      return await apiRequest("POST", "/api/rate-analysis-jobs", { preset });
    },
    onSuccess: () => {
      refetch();
      toast({
        title: "Job Started",
        description: "Rate analysis job has been queued and will run in the background",
      });
    },
    onError: (error: Error) => {
      toast({
        variant: "destructive",
        title: "Error",
        description: error.message || "Failed to start rate analysis job",
      });
    },
  });
  ...
  const cancelJobMutation = useMutation({
    mutationFn: async (jobId: string) => {
      return await apiRequest("POST", `/api/rate-analysis-jobs/${jobId}/cancel`);
    },
    onSuccess: () => {
      refetch();
      toast({
        title: "Job Cancelled",
        description: "Rate analysis job has been cancelled",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const jobs = rateAnalysisData?.jobs || [];
  ...
  return (
    <Card data-testid="card-rate-analysis-jobs">
      <CardHeader>
        <CardTitle className="text-lg flex items-center gap-2">
          <Truck className="h-5 w-5" />
          Smart Carrier Rate Analysis
        </CardTitle>
        <CardDescription>
          Analyze shipments to find cost-effective shipping alternatives. Jobs run in the background.
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <div className="rounded-md border p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">
                    {activeJob.status === 'running' ? 'Analyzing' : 'Pending'}: {getPresetLabel(activeJob.preset)}
                  </span>
                </div>
                <Button
                  variant="destructive"
                  ...
                  data-testid="button-cancel-rate-analysis"
                >
                  Cancel
                </Button>
              </div>
              {activeJob.status === 'running' && activeJob.shipmentsTotal > 0 && (
                <div className="space-y-1">
                  <Progress 
                    value={(activeJob.shipmentsAnalyzed / activeJob.shipmentsTotal) * 100} 
                    className="h-2"
                  />
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>{activeJob.shipmentsAnalyzed} / {activeJob.shipmentsTotal} shipments</span>
                    {activeJob.savingsFound && parseFloat(activeJob.savingsFound) > 0 && (
                      <span className="text-green-600 font-medium">
                        ${parseFloat(activeJob.savingsFound).toFixed(2)} potential savings
                      </span>
                    )}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
              {presets.map((preset) => (
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => createJobMutation.mutate(preset.value)}
                  disabled={createJobMutation.isPending}
                  data-testid={`button-rate-analysis-${preset.value}`}
                >
                  {preset.label}
                </Button>
              ))}
            ...
            <div className="space-y-2 pt-4 border-t">
              ...
              {jobs.slice(0, 5).map((job) => (
                <div 
                  ...
                  data-testid={`rate-job-item-${job.id}`}
                >
                  <div className="flex items-center gap-2">
                    <Badge 
                      variant={
                    ...
                    <span>{getPresetLabel(job.preset)}</span>
                  </div>
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-2">
      ...
      <Button
        ...
        variant="outline"
        onClick={async () => {
          setIsResync30Loading(true);
          try {
            await apiRequest('POST', '/api/operations/force-unified-resync-30');
            queryClient.invalidateQueries({ queryKey: ["/api/operations/queue-stats"] });
            toast({
              title: "30-Day Resync Started",
              description: "Cursor reset to 30 days ago.",
            });
          } catch (err) {
            console.error('Failed to force 30-day resync:', err);
            toast({
              title: "Failed to force 30-day resync",
              description: err instanceof Error ? err.message : "Unknown error",
              variant: "destructive",
            });
          } finally {
            setIsResync30Loading(false);
          }
        }}
        disabled={!credentialsConfigured || isResync30Loading}
        data-testid="button-force-unified-resync-30"
      >
        <RefreshCw className={cn("h-3 w-3 mr-1", isResync30Loading && "animate-spin")} />
        {isResync30Loading ? "Resetting..." : "Resync (30-day)"}
      </Button>
      <Button
        size="sm"
        variant="outline"
        onClick={async () => {
          ...
          try {
            ...
            console.error('Failed to force 90-day resync:', err);
            toast({
              title: "Failed to force 90-day resync",
              description: err instanceof Error ? err.message : "Unknown error",
              variant: "destructive",
            });
          } finally {
            setIsResync90Loading(false);
          }
        }}
        disabled={!credentialsConfigured || isResync90Loading}
        data-testid="button-force-unified-resync-90"
      >
