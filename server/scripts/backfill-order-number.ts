/**
 * Backfill script to populate order_number column from shipmentData JSONB
 * Extracts ShipStation shipment_number into dedicated order_number column
 */

import { db } from '../db';
import { shipments } from '@shared/schema';
import { sql } from 'drizzle-orm';

/**
 * Extract order_number from shipmentData JSONB
 * Returns the customer-facing order number (e.g., "JK3825345229")
 */
function extractOrderNumber(shipmentData: any): string | null {
  return shipmentData?.shipment_number || shipmentData?.shipmentNumber || null;
}

async function backfillOrderNumber() {
  console.log('========== ORDER_NUMBER BACKFILL STARTED ==========');
  console.log('Fetching all shipments with shipmentData...');

  // Fetch all shipments that have shipmentData
  const allShipments = await db
    .select()
    .from(shipments)
    .where(sql`${shipments.shipmentData} IS NOT NULL`);

  console.log(`Found ${allShipments.length} shipments with shipmentData`);

  let updatedCount = 0;
  let skippedCount = 0;
  let batchCount = 0;
  const BATCH_SIZE = 100;

  // Process in batches
  for (let i = 0; i < allShipments.length; i += BATCH_SIZE) {
    const batch = allShipments.slice(i, i + BATCH_SIZE);
    batchCount++;
    
    console.log(`Processing batch ${batchCount} (${batch.length} shipments)...`);

    for (const shipment of batch) {
      try {
        const orderNumber = extractOrderNumber(shipment.shipmentData);
        
        // Only update if we extracted an order number
        if (orderNumber) {
          await db
            .update(shipments)
            .set({ orderNumber })
            .where(sql`${shipments.id} = ${shipment.id}`);
          
          updatedCount++;
        } else {
          skippedCount++;
        }
      } catch (error: any) {
        console.error(`Error processing shipment ${shipment.id}:`, error.message);
      }
    }

    console.log(`Batch ${batchCount} complete. Updated: ${updatedCount}, Skipped: ${skippedCount}`);
  }

  console.log('========== BACKFILL COMPLETE ==========');
  console.log(`Total shipments processed: ${allShipments.length}`);
  console.log(`Updated with order_number: ${updatedCount}`);
  console.log(`Skipped (no order_number): ${skippedCount}`);
  console.log('==========================================');
}

// Run the backfill
backfillOrderNumber()
  .then(() => {
    console.log('Backfill completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Backfill failed:', error);
    process.exit(1);
  });
