...
const LABEL_TRACKING_SYNC_BATCH_SIZE = 5;
const LABEL_TRACKING_MIN_AGE_DAYS = 14;

/**
 * Sync tracking statuses using the label-based tracking endpoint.
 * 
 * PURPOSE: Catch shipments whose tracking status wasn't updated by webhooks.
 * Uses a two-step approach via the V2 labels API:
 *   1. GET /v2/labels?tracking_number={number} → get label_id
 *   2. GET /v2/labels/{label_id}/track → get status_code (DE, SP, IT, etc.)
 * 
 * CRITERIA:
 * - lifecycle_phase IN ('in_transit', 'on_dock') — only non-terminal shipped shipments
 * - tracking_number IS NOT NULL — need a tracking number to look up the label
 * - created_at older than 14 days — gives webhooks time to work first
 * - Sorted oldest first — clears backlog from the bottom up
 * 
 * COST: 2 API calls per shipment (label lookup + tracking), max 10 calls per cycle
 * 
 * TRIGGERS: After main poll cycle when caught up (same as other maintenance jobs)
 */
async function syncLabelTrackingStatuses(): Promise<{ processed: number; updated: number; errors: number; rateLimited: boolean }> {
  ...
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - LABEL_TRACKING_MIN_AGE_DAYS);
  ...
  const shipmentsToCheck = await db
    ...
    .orderBy(shipments.createdAt)
    .limit(LABEL_TRACKING_SYNC_BATCH_SIZE);

  if (shipmentsToCheck.length === 0) {
    return { processed: 0, updated: 0, errors: 0, rateLimited: false };
  }

  console.log(`[UnifiedSync] Label tracking sync: checking ${shipmentsToCheck.length} shipments (oldest first, ${LABEL_TRACKING_MIN_AGE_DAYS}+ days old)`);
  ...
  for (const shipment of shipmentsToCheck) {
    ...
    try {
      // Step 1: Look up label by tracking number to get label_id
      const labelsUrl = `${SHIPSTATION_API_BASE}/v2/labels?tracking_number=${encodeURIComponent(shipment.trackingNumber!)}`;
      const labelsResponse = await fetch(labelsUrl, {
        method: 'GET',
        headers: { 'api-key': apiKey, 'Content-Type': 'application/json' },
      });

      if (labelsResponse.status === 429) {
        console.log('[UnifiedSync] Hit rate limit during label tracking sync, stopping batch');
        rateLimited = true;
        break;
      }

      if (!labelsResponse.ok) {
        console.error(`[UnifiedSync] Label lookup failed for ${shipment.orderNumber}: ${labelsResponse.status}`);
        errors++;
        continue;
      }
      ...
      // Step 2: Get tracking status via label track endpoint
      const trackUrl = `${SHIPSTATION_API_BASE}/v2/labels/${encodeURIComponent(labelId)}/track`;
      ...
      if (!trackResponse.ok) {
        console.error(`[UnifiedSync] Track endpoint failed for ${shipment.orderNumber} (label ${labelId}): ${trackResponse.status}`);
        errors++;
        continue;
      }

      const trackingData = await trackResponse.json();
      const statusCode = trackingData.status_code ? String(trackingData.status_code).toUpperCase() : null;
      ...
      if (statusCode && statusCode !== (shipment.status?.toUpperCase() || null)) {
        await db
          ...
          .set({
            status: statusCode,
            statusDescription: trackingData.status_description || null,
            updatedAt: new Date(),
          })
          .where(eq(shipments.id, shipment.id));

        console.log(`[UnifiedSync] Label tracking update: ${shipment.orderNumber} status ${shipment.status} → ${statusCode} (phase: ${shipment.lifecyclePhase})`);
      ...
      console.error(`[UnifiedSync] Error in label tracking sync for ${shipment.orderNumber}:`, err.message);
      errors++;
    }
  }

  if (updated > 0 || errors > 0) {
    console.log(`[UnifiedSync] Label tracking sync complete: ${processed} checked, ${updated} updated, ${errors} errors`);
  }
...
async function runPollLoop(): Promise<boolean> {
  ...
  try {
    ...
    if (!needsImmediateFollowup && !result.hitRateLimit) {
      ...
      // Uses V2 label-based tracking endpoint: GET /v2/labels/{label_id}/track
      try {
        await syncLabelTrackingStatuses();
      } catch (trackingSyncErr) {
        console.error('[UnifiedSync] Label tracking sync error (non-fatal):', trackingSyncErr);
      }
    }
