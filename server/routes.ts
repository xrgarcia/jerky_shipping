...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/merge-groups/stats", requireAuth, async (req, res) => {
    try {
      const { getMergeGroupQueueStats } = await import('./services/merge-group-queue');
      ...
      const queueStats = await getMergeGroupQueueStats();

      res.json({
        groups: {
          detected: stateMap['detected'] || 0,
          mergeStarted: stateMap['merge_started'] || 0,
          mergeComplete: stateMap['merge_complete'] || 0,
          allSessioned: stateMap['all_sessioned'] || 0,
          total: Object.values(stateMap).reduce((a, b) => a + b, 0),
        },
        queue: queueStats,
      });
    } catch (error: any) {
      console.error("Error fetching merge group stats:", error.message);
      res.status(500).json({ error: "Failed to fetch merge group stats" });
    }
  });

  app.get("/api/merge-groups/by-shipment/:shipmentId", requireAuth, async (req, res) => {
    try {
      const shipmentId = req.params.shipmentId;

      const [member] = await db
        .select({
          mergeGroupId: mergeGroupMembers.mergeGroupId,
          role: mergeGroupMembers.role,
        })
        .from(mergeGroupMembers)
        .where(eq(mergeGroupMembers.shipmentId, shipmentId))
        .limit(1);

      if (!member) return res.json({ group: null });

      const [group] = await db.select().from(mergeGroups)
        .where(eq(mergeGroups.id, member.mergeGroupId)).limit(1);

      res.json({ group, role: member.role });
    } catch (error: any) {
      console.error("Error fetching merge group by shipment:", error.message);
      res.status(500).json({ error: "Failed to fetch merge group" });
    }
  });

  app.get("/api/merge-groups/:id", requireAuth, async (req, res) => {
    try {
      ...
      const members = await db
        .select({
          id: mergeGroupMembers.id,
          shipmentId: mergeGroupMembers.shipmentId,
          orderNumber: mergeGroupMembers.orderNumber,
          role: mergeGroupMembers.role,
          originalItemCount: mergeGroupMembers.originalItemCount,
          originalItems: mergeGroupMembers.originalItems,
        ...
        .from(mergeGroupMembers)
        .leftJoin(shipments, eq(mergeGroupMembers.shipmentId, shipments.id))
        .where(eq(mergeGroupMembers.mergeGroupId, groupId))
        .orderBy(mergeGroupMembers.role);

      const membersWithCurrentItems = await Promise.all(members.map(async (m) => {
      ...
      res.json({ group, members: membersWithCurrentItems });
    } catch (error: any) {
      console.error("Error fetching merge group:", error.message);
      res.status(500).json({ error: "Failed to fetch merge group" });
    }
  });

  app.get("/api/merge-groups", requireAuth, async (req, res) => {
    try {
      const page = Math.max(1, parseInt(req.query.page as string) || 1);
      const limit = Math.min(100, Math.max(1, parseInt(req.query.limit as string) || 25));
      const offset = (page - 1) * limit;
      const stateFilter = req.query.state as string;
      const sortBy = (req.query.sortBy as string) || 'detectedAt';
      const sortOrder = (req.query.sortOrder as string) === 'asc' ? asc : desc;

      const conditions: SQL[] = [];
      if (stateFilter && stateFilter !== 'all') {
        conditions.push(eq(mergeGroups.state, stateFilter));
      }
      ...
      const sortColumnMap: Record<string, any> = {
        ...
        createdAt: mergeGroups.createdAt,
      };
      const sortColumn = sortColumnMap[sortBy] || mergeGroups.detectedAt;

      const [groups, countResult] = await Promise.all([
        db.select().from(mergeGroups)
          .where(whereClause)
          .orderBy(sortOrder(sortColumn))
          .limit(limit).offset(offset),
        db.select({ count: sql<number>`COUNT(*)::int` }).from(mergeGroups).where(whereClause),
      ]);
      ...
      console.error("Error fetching merge groups:", error.message);
      res.status(500).json({ error: "Failed to fetch merge groups" });
    }
  });

  // ================================================================
  // Merge Group Queue endpoints
  // ================================================================

  app.get("/api/merge-group-queue/stats", requireAuth, async (req, res) => {
    try {
      const { getMergeGroupQueueStats } = await import('./services/merge-group-queue');
      const stats = await getMergeGroupQueueStats();
      res.json(stats);
    } catch (error: any) {
      console.error("Error fetching merge group queue stats:", error.message);
      res.status(500).json({ error: "Failed to fetch queue stats" });
    }
  });

  app.get("/api/merge-group-queue/jobs", requireAuth, async (req, res) => {
    try {
      const page = Math.max(1, parseInt(req.query.page as string) || 1);
      const limit = Math.min(100, Math.max(1, parseInt(req.query.limit as string) || 25));
      const offset = (page - 1) * limit;
      ...
      const conditions: SQL[] = [];
      if (statusFilter && statusFilter !== 'all') {
        conditions.push(eq(mergeGroupQueue.status, statusFilter));
      }
      if (orderNumberFilter && orderNumberFilter.trim()) {
        conditions.push(ilike(mergeGroupQueue.orderNumber, `%${orderNumberFilter.trim()}%`));
      }

      const whereClause = conditions.length > 0 ? and(...conditions) : undefined;

      const sortColumnMap: Record<string, any> = {
        createdAt: mergeGroupQueue.createdAt,
        id: mergeGroupQueue.id,
        status: mergeGroupQueue.status,
        retryCount: mergeGroupQueue.retryCount,
        processedAt: mergeGroupQueue.processedAt,
        completedAt: mergeGroupQueue.completedAt,
      };
      const sortColumn = sortColumnMap[sortBy] || mergeGroupQueue.createdAt;
      const orderFn = sortOrder === 'asc' ? asc : desc;

      const [jobs, countResult] = await Promise.all([
        db.select().from(mergeGroupQueue)
          .where(whereClause)
          .orderBy(orderFn(sortColumn))
          .limit(limit).offset(offset),
        db.select({ count: sql<number>`COUNT(*)::int` }).from(mergeGroupQueue).where(whereClause),
      ]);
      ...
      console.error("Error fetching merge group queue jobs:", error.message);
  ...
  app.get("/api/qc-explosion-queue/stats", requireAuth, async (req, res) => {
    try {
      const { getQcExplosionQueueStats } = await import("./services/qc-explosion-queue");
      const stats = await getQcExplosionQueueStats();
      res.json(stats);
    } catch (error: any) {
      console.error("Error getting QC explosion queue stats:", error.message);
      res.status(500).json({ error: "Failed to get QC explosion queue stats" });
    }
  });

  app.get("/api/qc-explosion-queue/jobs", requireAuth, async (req, res) => {
    try {
      const page = Math.max(1, parseInt(req.query.page as string) || 1);
      const limit = Math.min(100, Math.max(1, parseInt(req.query.limit as string) || 25));
      const offset = (page - 1) * limit;
      ...
      const conditions: SQL[] = [];
      if (statusFilter && statusFilter !== "all") {
        conditions.push(eq(qcExplosionQueue.status, statusFilter));
      }
      if (orderNumberFilter && orderNumberFilter.trim()) {
        conditions.push(ilike(qcExplosionQueue.orderNumber, `%${orderNumberFilter.trim()}%`));
      }
      ...
      const sortColumnMap: Record<string, any> = {
        ...
        processedAt: qcExplosionQueue.processedAt,
      };
      const sortColumn = sortColumnMap[sortBy] || qcExplosionQueue.createdAt;
      const orderFn = sortOrder === "asc" ? asc : desc;
      ...
      console.error("Error getting QC explosion queue jobs:", error.message);
      res.status(500).json({ error: "Failed to get QC explosion queue jobs" });
    }
  });
  ...
  // Customer Shipping Methods API - List all customer shipping methods
  app.get("/api/settings/customer-shipping-methods", requireAuth, async (req, res) => {
    try {
      const { customerShippingMethods } = await import("@shared/schema");
      const methods = await db.select().from(customerShippingMethods).orderBy(customerShippingMethods.name);
      res.json(methods);
    } catch (error) {
      console.error("Error fetching customer shipping methods:", error);
      res.status(500).json({ error: "Failed to fetch customer shipping methods" });
    }
  });

  // Customer Shipping Methods API - Update a customer shipping method
  app.put("/api/settings/customer-shipping-methods/:id", requireAuth, async (req, res) => {
    try {
      const { customerShippingMethods } = await import("@shared/schema");
      const { id } = req.params;
      ...
      const result = await db
        ...
        .where(eq(customerShippingMethods.id, parseInt(id)))
        .returning();
      
      if (result.length === 0) {
        return res.status(404).json({ error: "Customer shipping method not found" });
      }
      
      res.json(result[0]);
    } catch (error) {
      console.error("Error updating customer shipping method:", error);
      res.status(500).json({ error: "Failed to update customer shipping method" });
    }
  });

  // Customer Shipping Methods API - Sync new methods from shipments
  app.post("/api/settings/customer-shipping-methods/sync", requireAuth, async (req, res) => {
    try {
      const { customerShippingMethods, shipments: shipmentsTable } = await import("@shared/schema");
      
      // Get all unique service codes from shipments that aren't already in customer_shipping_methods
      const result = await db.execute(sql`
        INSERT INTO customer_shipping_methods (name)
      ...
      console.error("Error syncing customer shipping methods:", error);
      res.status(500).json({ error: "Failed to sync customer shipping methods" });
    }
  });
  ...
  app.get("/api/forecasting/kit-timeseries", async (req: Request, res: Response) => {
    try {
      const { TimeRangePreset } = await import('@shared/forecasting-types');
      const { forecastingService } = await import('./services/forecasting-service');
      ...
      console.error("[Forecasting] Error fetching kit timeseries:", error);
      res.status(500).json({ error: "Failed to fetch kit timeseries: " + error.message });
    }
  });

  app.get("/api/forecasting/summary-metrics", async (req: Request, res: Response) => {
    try {
      const { TimeRangePreset } = await import('@shared/forecasting-types');
      const { forecastingService } = await import('./services/forecasting-service');
      ...
      console.error("[Forecasting] Error fetching summary metrics:", error);
      res.status(500).json({ error: "Failed to fetch summary metrics: " + error.message });
    }
  });
