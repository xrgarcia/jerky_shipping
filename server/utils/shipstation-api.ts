...
/**
 * Get tracking status for a shipment via the dedicated tracking endpoint.
 * Uses GET /v2/tracking?carrier_code={code}&tracking_number={number}
 * 
 * Key status_code values:
 *   DE = Delivered to final address
 *   SP = Service Point (delivered to locker/pickup location)
 *   IT = In Transit
 *   UN = Unknown (carrier has no tracking info)
 *   EX = Exception (weather delay, damaged label, incorrect address)
...
export async function getTrackingStatus(
  carrierCode: string,
  trackingNumber: string
): Promise<ApiResponseWithRateLimit<TrackingStatusResponse | null>> {
  if (!SHIPSTATION_API_KEY) {
    throw new Error('SHIPSTATION_API_KEY environment variable is not set');
  }

  const url = `${SHIPSTATION_API_BASE}/v2/tracking?carrier_code=${encodeURIComponent(carrierCode)}&tracking_number=${encodeURIComponent(trackingNumber)}`;
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(`ShipStation tracking API error: ${response.status} ${errorText}`);
  }
...
const KNOWN_CARRIER_PREFIXES = ['usps', 'ups', 'fedex', 'dhl', 'stamps_com', 'endicia'];

/**
 * Derive carrier_code from a service_code string.
 * ShipStation service codes follow the pattern: {carrier}_{service_name}
 * e.g. "usps_ground_advantage" → "usps", "ups_ground" → "ups"
 * 
 * Falls back to the cached service_code → carrier_id map if the prefix
 * doesn't match a known carrier.
 */
export function deriveCarrierCodeFromServiceCode(serviceCode: string): string | null {
  if (!serviceCode) return null;

  const lower = serviceCode.toLowerCase();

  for (const prefix of KNOWN_CARRIER_PREFIXES) {
    if (lower.startsWith(prefix + '_') || lower === prefix) {
      return prefix;
    }
  }
