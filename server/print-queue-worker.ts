...
/**
 * Check for stale print jobs and update metrics
...
export async function checkStaleJobs(): Promise<StaleJobsMetrics> {
  ...
  try {
    ...
    workerStats.lastProcessedCount = activeJobs.length;
    workerStats.lastCompletedAt = new Date();

    // Log stale job status
    if (staleJobs.length > 0) {
      log(`Found ${staleJobs.length} stale job(s): ${warningCount} warning, ${criticalCount} critical`);
    ...
    if (statusChanged || countChanged) {
      log(`Broadcasting stale jobs update: ${overallHealth} (${staleJobs.length} stale)`);
